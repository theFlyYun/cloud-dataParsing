var debug = require('debug')('deviceShadow:mqClient:kafka');
...
var cloudLibUtils = require('cloud-lib').utils;

//monitor count
var fromClientBytesCount = monitorManager.registCountVar('fromClientBytes');
...
function MqClient(opts) {
  ...
  var onMessage = function (res) {
    ...
    appMonitorHelper.onKafkaMessageReceive(res2);
    try {
  ...
  this._consumerGroup.on('connect', onConnect.bind(this));//above 2 only one will work
  ...
  var op = Promise.map(payloads, function (item) {
    ...
    return new Promise(function (resolve, reject) {
      producer.send(payloads, function (err, data) {
        ...
        appMonitorHelper.onKafkaReqsSent(payloads);
        return resolve(data);
      });
    });
  });
...
prototype.sendMqttPublish = function (mqttTopic, mqttPayload, params) {
  var _this = this;
  ...
  return checkInput(arguments).then(function () {
    ...
    //from Clinet bytes
    var bytes = cloudLibUtils.getStringSize(JSON.stringify(message));
    fromClientBytesCount.addCount(bytes);
    return _this.publish(topic, message);
  });
};
